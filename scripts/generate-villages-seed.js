...
// 4. Street-qualified overrides. The legacy dashboard source can only express
// subdivision matching, so rows that additionally require a street match are
// maintained here and appended AFTER the parsed entries (higher order wins in
// matchVillage, so these take precedence over their generic siblings).
// streetPattern semantics: the row matches only when the subdivision contains
// matchPattern AND the listing's street text contains streetPattern.
const STREET_QUALIFIED = [
    {
        // Subdivision 'BAY ISLES' on a Bayou-named street (Bayou Road, Bayou
        // Sound, Bayou Circle) is The Bayou, not the Harbor Section.
        matchPattern: 'bay isles',
        streetPattern: 'bayou',
        villageName: 'The Bayou (Bay Isles)',
        villageSortHelp: 'The Bayou (Bay Isles)',
        villageURL: 'https://www.lifeinlongboatkey.com/neighborhood/the-bayou-bay-isles',
        village1: '98eb305d-0b83-496c-8b87-d5982cef505c'
    }
];
STREET_QUALIFIED.forEach((e, i) => { e.order = entries.length + 1 + i; });
entries.push(...STREET_QUALIFIED);
