...
function applyFilters() {
  ...
  // Reset to the first page of 9 whenever the filters change (Load more may
  // have raised the page size), then apply the filter. Skip the reset when
  // the size is already 9: setPageSize triggers its own fetch, so resetting
  // unconditionally made every filter change hit the collection twice.
  const resetPageSize = $w(DATASET_ID).getPageSize() === PAGE_SIZE
    ? Promise.resolve()
    : $w(DATASET_ID).setPageSize(PAGE_SIZE);
