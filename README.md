...
- **`imagesFailed` is the same number run after run** - one Stagging row keeps failing; `listingsHealth.stuckStagging` names it, and its `photos_failed` events carry the URLs and the error. If the error is `HTTP 429`, MLSGrid's media server is rate-limiting a large gallery: the drain backs off and continues at lower concurrency, so the row completes over a few attempts on its own.
